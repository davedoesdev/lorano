                ...
                while (true)
                {
                    ...
                    while ((msg = payload.rxpk.shift()) !== undefined)
                    {
                        ...
                        if (decoded.isJoinRequestMessage())
                        {
                            ...
                            // Make NwkSKey
                            let cipher = crypto.createCipheriv('aes-128-ecb', device.AppKey, '');
                            let nwk_skey = [];
                            ...
                            nwk_skey = Buffer.concat(nwk_skey);

                            // Make AppSKey
                            cipher = crypto.createCipheriv('aes-128-ecb', device.AppKey, '');
                            let app_skey = [];
                            ...
                            console.log("UPDATED", device.NwkAddr);
                        ...
                        if (decoded.getMType() !== 'Unconfirmed Data Up')
                        {
                            continue;
                        }

                        let skeys;
                        // Check if from OTAA device (7 msb of DevAddr match
                        // 7 lsb of netid (NwkID))
                        let otaa = (buffers.DevAddr[0] >> 1) ===
                                   (this._options.netid[3] & 0x7f);
                        if (otaa)
                        {
                            let nwk_addr = Buffer.from(buffers.DevAddr);
                            nwk_addr[0] &= 0x01;
                            console.log("GETTING", nwk_addr);
                            skeys = await this._OTAASessions
                                .query()
                                .findById(nwk_addr);
