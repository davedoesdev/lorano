...
module.exports = class extends Duplex
{
    constructor(Model, uplink, downlink, options)
    {
        ...
        this._OTAAKeys = class extends Model {
            static get tableName() { return 'OTAAKeys'; };
            static get idColumn() { return 'DevEUI'; };
        };
        ...
        (async () =>
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    while ((msg = payload.rxpk.shift()) !== undefined)
                    {
                        ...
                        console.log("BUFFERS", buffers);
                            ...
                            const akeys = await this._OTAAKeys 
                                .query()
                                .findById(buffers.DevEUI);
                            if (!akeys)
                            {
                                continue;
                            }
                            ...
                            // Verify request
                            if (!lora_packet.verifyMIC(decoded, null, akeys.AppKey))
                            {
                            ...
                            // Make DevAddr
                            let dev_addr = crypto.randomBytes(4);
                            // 7 msb of devaddr match 7 lsb of netid (NwkID)
                            dev_addr[0] &= 0x80;
                            dev_addr[0] |= (this._options.netid[3] & 0x7f);

                            // Make NwkSKey
                            let nwk_skey = [];
                            let cipher = crypto.createCipheriv('aes-128-ecb', akeys.AppKey, '');
                            nwk_skey.push(cipher.update(Buffer.from([0x01])));
                            nwk_skey.push(cipher.update(app_nonce));
                            nwk_skey.push(cipher.update(this._options.netid));
                            nwk_skey.push(cipher.update(buffers.DevNonce));
                            nwk_skey.push(cipher.update(Buffer.alloc(7)));
                            nwk_skey.push(cipher.final());

                            // Make AppSKey
                            let app_skey = [];
                            cipher = crypto.createCipheriv('aes-128-ecb', akeys.AppKey, '');
                            app_skey.push(cipher.update(Buffer.from([0x02])));
                            app_skey.push(cipher.update(app_nonce));
                            app_skey.push(cipher.update(this._options.netid));
                            app_skey.push(cipher.update(buffers.DevNonce));
                            app_skey.push(cipher.update(Buffer.alloc(7)));
                            app_skey.push(cipher.final());

                            // Delete existing session
                            let session = await this._OTAASessions
                                .query()
                                .findById(buffers.DevEUI);
                            if (session)
                            {
                                ...
                                await this._OTAASessions
                                    .query()
                                    .deleteById(buffers.DevEUI);

                                // Delete session keys
                                await this._SessionKeys
                                    .query()
                                    .deleteById(session.DevAddr);
                            }
                            ...
                            }, app_skey, nwk_skey, akeys.AppKey);

                            console.log(encoded);

                            // reply
                            // emit join event
                            // or method so can modify reply
                            // function to devaddr->deveui?

                            continue;
                        }

                        // TODO: Check this is data message
                        console.log("TYPE", decoded.getMType());
